...
function [tempK23, tempB23, tempK24, tempB24] = timeFitting(anchorInteractionSeqMatrix, anchorInteractionTimeMatrix, window, tempK23, tempK24, tempB23, tempB24)
    ...
    if seq12 == seq13 && seq13 == seq14 && seq12 ~= lastIndex
        % 基站2，3，4均收到来自基站1的信号
        if length(find(anchorInteractionTimeMatrix(1, 2:4, seq12) ~= 0)) == 3
            ...
            % 构建用于线性拟合的矩阵
            tempY23Matrix(seqCount) = anchorInteractionTimeMatrix(1,3,seq12);
            tempY24Matrix(seqCount) = anchorInteractionTimeMatrix(1,4,seq12);
            tempX2Matrix(seqCount) = anchorInteractionTimeMatrix(1,2,seq12);
            seqCount = seqCount + 1;
